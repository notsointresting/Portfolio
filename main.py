...
with st.container():
    ...
    def display_project(col, project):
        with col:
            st.markdown(
                ...
                f'<img src="{project["image_url"]}" style="width:100%;height:200px;"></a>',
                unsafe_allow_html=True,
            )
    ...
    # Assuming you have defined columns and projects somewhere before this function

    # Calculate the number of rows needed
    num_rows = -(-len(projects) // 3)  # Equivalent to ceil(len(projects) / 3)

    # Display projects in each row
    for i in range(num_rows):
        row = st.columns(3)
        for j in range(3):
            index = i * 3 + j
            if index < len(projects):
                display_project(row[j], projects[index])
            else:
                break
...
st.write("---")
with st.container():  
    col1,col3 = st.columns([0.475, 0.05])
